...
public class ConsumatoreDao implements ConsumatoreInterface<ConsumatoreBean> {
  ...
  /**
   * Metodo utilizzato per eliminare una riga identificata da un bean all'interno del database.
   * 
   ...
   */
  public Collection<ConsumatoreBean> doRetrieveForTracciamento(String codiceFiscale,
      String dataIniziale) {
    Connection con = null;
    PreparedStatement statement = null;
    ArrayList<ConsumatoreBean> listaRisultati = new ArrayList<ConsumatoreBean>();
    // codicefiscale, data, fascia, boolean
    String table = "consumatore c1 JOIN prenotazione p1 ON p.email = c.email";
    String dateMalate = "SELECT dataPrenotazione FROM " + table
        + " WHERE codicefiscale = ? AND campobooleano = true";
    String table2 =
    ...
    String consumatoriEntrati =
        "SELECT email, nome, cognome, fascia, sala, dataPrenotazione, c2.campobooleano FROM "
            + table2 + " WHERE dataPrenotazione in (" + dateMalate + ")";
    try {
      con = DriverManagerConnectionPool.getConnection();
      ...
      while (results.next()) {
        ...
        if (fourteenDaysBetween(dataIniziale, dataPrenotazione)) {
          if(results.getBoolean("c2.campobooleano")) {
