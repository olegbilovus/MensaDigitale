...
import storage.interfaces.ModelInterface;


public class ValutazioneDao implements ModelInterface<ValutazioneBean, String> {
  ...
  */
  ...
  @Override
  public ValutazioneBean doRetrieveByKey(String email) throws SQLException {
  ...
  */
  ...
  public void doDelete(ValutazioneBean bean) throws SQLException {
    String sql = "DELETE FROM valutazione WHERE emaill=? AND piatto=?";
    try (Connection con = DriverManagerConnectionPool.getConnection();
        PreparedStatement statement = con.prepareStatement(sql);) {
      statement.setString(1, bean.getEmail());
